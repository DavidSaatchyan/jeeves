...
class TestConfigureCliniko:
    def _patch_discover(self, shard: str | None = "au1"):
        return patch("app.admin.integrations_hub._discover_shard", return_value=shard)
    ...
    def test_saves_api_key_and_detected_shard(self, client, mock_tenant):
        with self._patch_discover("eu1"):
            resp = client.post("/admin/api/integrations/crm/configure", json={
                "provider": "cliniko", "api_key": "cliniko-key-123",
            })
    ...
    def test_sets_user_agent(self, client, mock_tenant):
        with self._patch_discover():
            client.post("/admin/api/integrations/crm/configure", json={
                "provider": "cliniko", "api_key": "k",
            })
...
class TestClinikoStatus:
    def test_returns_not_connected_when_empty(self, client, mock_tenant):
        ...
        assert resp.status_code == 200
        cliniko = [i for i in resp.json()["integrations"] if i["id"] == "cliniko"][0]
        assert cliniko["status"] == "not_configured"
    ...
    def test_returns_connected_when_configured(self, client, mock_tenant):
        ...
        assert resp.status_code == 200
        cliniko = [i for i in resp.json()["integrations"] if i["id"] == "cliniko"][0]
        assert cliniko["status"] == "connected"
...
class TestShardFromKey:
    def test_extracts_shard_from_key_suffix(self):
        from app.admin.integrations_hub import _shard_from_key
...
class TestDiscoverShard:
    def test_uses_shard_from_key_when_valid(self):
        from app.admin.integrations_hub import _discover_shard
        with patch("app.admin.integrations_hub._try_shard", return_value=True) as mock_try:
    ...
    def test_falls_back_to_probe_when_no_suffix(self):
        from app.admin.integrations_hub import _discover_shard
        with patch("app.admin.integrations_hub._try_shard") as mock_try:
    ...
    def test_returns_none_when_all_fail(self):
        from app.admin.integrations_hub import _discover_shard
        with patch("app.admin.integrations_hub._try_shard", return_value=False):
            result = _discover_shard("key")
        assert result is None

    def test_try_shard_correct_url_and_auth(self):
        from app.admin.integrations_hub import _try_shard
        with patch("app.admin.integrations_hub.httpx.get") as mock_get:
